<?php $page_title = 'Edit mahasiswa'; include 'header.php';
$id = $_GET['id'] ?? 0;
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($password)) $conn->query("UPDATE users SET password='".md5($password)."' WHERE id=(SELECT user_id FROM mahasiswa WHERE id=$id)");
    $conn->query("UPDATE mahasiswa SET nidn='$nidn', nama='$nama' WHERE id=$id");
    header('Location: mahasiswa.php'); exit;
}
$data = $conn->query("SELECT d.*, u.username FROM mahasiswa d JOIN users u ON d.user_id = u.id WHERE d.id=$id")->fetch_assoc();
if (!$data) { header('Location: mahasiswa.php'); exit; }
?>
<div class="card"><div class="card-header">Edit mahasiswa</div><div class="card-body">
<form method="POST">
    ...
    <button type="submit" class="btn btn-success">Update</button> <a href="mahasiswa.php" class="btn btn-secondary">Batal</a>
</form></div></div>
